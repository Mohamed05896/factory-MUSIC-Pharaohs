...
DEVS = ["DEV_Abdullah", "DEV_AH6"] #يوزرات المطورين المصنع
...
async def start(client, message):
   if message.from_user.username in DEVS:
     ...
     return await message.reply_text("اهلا بيك في مصنع بوتات عبدالله ميوزك", reply_markup=kep)
   kep = ReplyKeyboardMarkup([["صنع بوت", "حذف بوت"], ["السورس"]], resize_keyboard=True)
   await message.reply_text("اهلا بيك ف مصنع بوتات عبدالله ميوزك", reply_markup=kep)
...
async def alivehi(client: Client, message):
    keyboard = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton("𝙶𝚁𝙾𝚄𝙿️", url=f"https://t.me/VIP3V_bot"),
                InlineKeyboardButton("𝚂𝙾𝚄𝚁𝙲𝙴️", url=f"https://t.me/UI_SB"),
            ],
            [
                 InlineKeyboardButton(f"𝗗𝗘𝗩 𝗕I𝗗𝗨 💎", url=f"https://t.me/DEV_SASA")
            ]
        ]
    )
